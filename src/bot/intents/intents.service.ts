...
export class IntentsService {
  constructor(@InjectModel(Intents.name) private readonly model: Model<IntentsDocument>) {}

  async findAll(): Promise<Intents[]> {
  ...
  async create(createIntents: CreateIntents): Promise<{message: string,statusCode: number, intents: Intents}> {
  ...
  async delete(id: string): Promise<{message: string,statusCode: number, intents: Intents}> {
